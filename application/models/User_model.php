...
public function register($fname,$lname,$mobile,$alternate_mobile,$dob,$email,$role,$status,$current_address,$permanent_address,$image){
  $obj = new stdClass();
  // if(trim($email, " ")){
  // if($this->user_model->checkUserEmail($email)->value){
    // if($this->user_model->checkUser($username)->value){
 $query = $this->db->query("insert into user (`name`, `first_name`, `last_name`, `mobile`, `alternate_mobile`, `role`,`status`, `email`, `dob`,`current_address`,`permanent_address`,`image`)values('$fname.' '.$lname','$fname','$lname','$mobile','$alternate_mobile',$role,$status,'$email','$dob','$current_address','$permanent_address','$image')");
 if($query){
  $id=$this->db->insert_id();
...
public function getAllUserData(){
  $query = $this->db->query("SELECT u.`id`, u.`name`, u.`mobile`, u.`email`, (SELECT number FROM idproof where idproof.id_type='pancard' AND u.id=idproof.cust_id LIMIT 1) AS 'Pancard' , (SELECT number FROM idproof where idproof.id_type='aadharcard' AND u.id=idproof.cust_id LIMIT 1) AS 'aadharcard' FROM `user` u WHERE 1")->result_array();
  if($query){
    $obj->value = true;
    $obj->data =$query ;
    return $obj ;
...
// public function deleteIdProofImage($id,$image_name){
//   $query= $this->db->query("delete from idproof_images where id=$id");
//     $obj = new stdClass();
//      if ($this->db->affected_rows() != 1){
//        $obj->value = false;
//       $obj->message ="Deletion failed" ;
